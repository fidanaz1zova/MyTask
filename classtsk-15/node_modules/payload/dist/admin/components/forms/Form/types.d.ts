...
import type { Condition, Field, Field as FieldConfig, Validate } from '../../../../fields/config/types';
export type Row = {
    blockType?: string;
...
export type FieldAction = ADD_ROW | DUPLICATE_ROW | MODIFY_CONDITION | MOVE_ROW | REMOVE | REMOVE_ROW | REPLACE_ROW | REPLACE_STATE | SET_ALL_ROWS_COLLAPSED | SET_ROW_COLLAPSED | UPDATE;
export type FormFieldsContext = [Fields, Dispatch<FieldAction>];
export type Context = {
    addFieldRow: ({ data, path, rowIndex, }: {
        data?: Data;
        path: string;
        rowIndex?: number;
    }) => Promise<void>;
    ...
    replaceFieldRow: ({ data, path, rowIndex, }: {
        data?: Data;
        path: string;
        rowIndex: number;
    }) => Promise<void>;
